...
const scripts = {
  ...
  diskCleanUp: async function () {
    ...
    const commands = [];
    keys.forEach((key) => {
      if (!key.includes("DownloadsFolder")) {
        //excluse downloads folder
        const command = `reg.exe ADD "${key}" /v ${value} /t ${type} /d ${data} /f`;
        commands.push(command);
      }
    });

    const sages = commands.join(" & "); //commands joined by & can be executed with a single child process
    const final = `${sages} && cleanmgr.exe /sagerun:1`;
  ...
  formatDrive: function (driveLetter, fileSystem) {
    if (fileSystem) {
      return execSync(
        "echo y | format " + driveLetter + ": /fs:" + fileSystem + " /q"
      );
    } else {
