...
const set = {
    ...
    Administrator: async function () {
        ...
        if (active.length === 0) {
            throw new Error('Could not determine administrator account status from net user output');
        }
        const parsed = active[0].split('               ')
        if (parsed[1] === 'Yes') return await core.elevate('net user administrator /active:no')
        return await core.elevate('net user administrator /active:yes')
    },
    ...
    WindowsDefender: async function (action) {
        ...
        await core.elevate(`${command1} & ${command2} & ${command3} & ${command4}`)
        */

        switch (action) {
            case 'on':
            case 'On':
                await core.elevate('sc config WinDefend start = enabled');
                await core.elevate('sc start WinDefend');
                break;
            case 'off':
            case 'Off':
                await core.elevate('sc config WinDefend start = disabled');
                await core.elevate('sc stop WinDefend');
                break;
            default:
