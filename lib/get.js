...
var get = {
  MBName: function () {
    const x = execSync(
      `${powershell} -command "Get-CimInstance -Query 'Select * from Win32_BaseBoard'"`
    )
      .toString()
      .trim();
  ...
  MemSpeed: function () {
    const x = execSync(
      `${powershell} -command "Get-CimInstance -Query 'Select * from Win32_PhysicalMemory'"`
    )
      .toString()
      .trim();
    const y = x
  ...
  GPUName: function () {
    const x = execSync(
      `${powershell} -command "Get-CimInstance -Query 'Select * from win32_VideoController'"`
    )
      .toString()
      .trim();
  ...
  CPUName: function () {
    const x = execSync(
      `${powershell} -command "Get-CimInstance -Query 'Select * from Win32_Processor'"`
    )
      .toString()
      .trim();
  ...
  Drives: function () {
    const x = execSync(
      `${powershell} -command "Get-CimInstance -Query 'Select * from Win32_LogicalDisk'"`
    )
      .toString()
      .trim();
    const y = x
  ...
  PowerGUID: function (a) {
    ...
    for (var i = 0; i < list.length; i++) {
      if (list[i].includes(option)) {
        guid = list[i]
          .substring(list[i].lastIndexOf(":") + 1, list[i].lastIndexOf("("))
          .trim();
      }
    }
  ...
  PowerScheme: function () {
    ...
    var scheme = output
      .substring(output.lastIndexOf(":") + 1, output.lastIndexOf("("))
      .trim();
    return scheme;
  },
  ...
  ScreenResolution: function () {
    const x = execSync(
      `${powershell} -command "Get-CimInstance -Query 'Select * from Win32_DesktopMonitor'"`
    )
      .toString()
      .trim();
    const y = x
  ...
  PublicIP: function () {
    var x = execSync("nslookup myip.opendns.com resolver1.opendns.com")
      .toString()
      .trim();
    var y = x.split("\r\n");
  ...
  SerialKey: function () {
    ...
    const y = x
      ...
      .filter(
        (line) =>
          !line.includes("--") && !line.includes("OA3xOriginalProductKey")
      )[0];
    if (y != undefined) return y;
    else
      return "This system is either activated through a digital license or is not activated.";
  },

  Model: function () {
    const x = execSync(
      `${powershell} -command "Get-CimInstance -Query 'Select * from Win32_ComputerSystemProduct'"`
    )
      .toString()
      .trim();
