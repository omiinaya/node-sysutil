...
var scripts = {
    ...
    toggleAdministrator: function() {
        if (getAdminStatus() === 'No') {
            return elevated('net user administrator /active:yes').toString().trim()
        } else {
            return elevated('net user administrator /active:no').toString().trim()
        }
        //troubleshoot why program does not exit after
    },
