...
const set = {
  PCName: async function (a) {
    ...
    const result = await core.elevate(
      `${powershell} -command "Rename-Computer -NewName '${a}'"`
    );
  ...
  PowerPlan: function (a) {
    ...
    let guid;
    try {
      guid = get.PowerGUID(a);
    } catch {
      run.registerPowerPlan(a);
      guid = get.PowerGUID(a);
    }
    console.log("Power Mode: " + a);
    return execSync("powercfg /setactive " + guid);
  },
