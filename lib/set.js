...
var set = {
    ...
    PCName: async function (a) {
        //does not fail, but does not work ???
        const query = execSync(`wmic computersystem get name`).toString()
        const current = await query.replace('Name', '').trim()
        core.elevate(`wmic computersystem where name="${current}" call rename name="${a}"`)
    },

    PCDescription: function (a) {
        core.elevate(`net config server /srvcomment:"${a}"`)
    },
