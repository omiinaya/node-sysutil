...
var core = {
  ...
  elevate: async function (a) {
    try {
      this.isAdmin(); //if not admin, elevate
  ...
  psDir: async function (key, value) {
    const key = "HKCR\\Microsoft.PowerShellConsole.1";
    const value = "FriendlyTypeName";
    const query = await this.regQueryValue(key, value); // receives string with results
    return query;
  },
};
